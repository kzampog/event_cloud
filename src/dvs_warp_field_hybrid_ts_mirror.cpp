...
//    Scalar max_correspondence_dist_sq = 0.1*0.1;
    Scalar max_correspondence_dist_sq = std::numeric_limits<Scalar>::infinity();
...
//    cilantro::KDTree<Scalar,Dim + 1>(frame2.points).search(frame2.points, cilantro::KNNNeighborhoodSpecification(12), regularization_nn);
    ...
    cilantro::VectorSet3f pf1 = frame1.points;
    pf1.row(2) *= 100;
    cilantro::VectorSet3f pf2 = frame2.points;
    pf2.row(2) *= 100;

    std::vector<cilantro::NeighborSet<Scalar>> regularization_nn;
    cilantro::KDTree<Scalar,Dim + 1>(pf2).search(pf2, cilantro::KNNNeighborhoodSpecification(12), regularization_nn);

    HybridPointsAdaptor<float,3> ff1(pf1);
    HybridPointsAdaptor<float,3> ff2(pf2);
...
int main(int argc, char ** argv) {
    ...
    double end = start + frame_duration;

    auto ts = events.row(2);

    cilantro::PointCloud3f frame1 = getFrame<3>(events, getIndexByTS(ts, start), getIndexByTS(ts, end));
    frame1.points.row(2) = frame1.points.row(2).array() - start;

    float max_ts = frame1.points.row(2).maxCoeff();
    cilantro::PointCloud3f frame2(frame1);
    for (size_t i = 0; i < frame2.size(); i++) {
        frame2.points(2,i) = max_ts - frame2.points(2,i);
    }
    ...
    timer.start();
//    auto wf = getDenseWarpField<cilantro::AffineTransform2f>(frame2, frame1, iter);
    auto wf = getDenseWarpField<cilantro::RigidTransform2f>(frame2, frame1, iter);
//    auto wf = getSparseWarpField<cilantro::AffineTransform2f>(frame2, frame1, iter);
//    auto wf = getSparseWarpField<cilantro::RigidTransform2f>(frame2, frame1, iter);
    timer.stop();
    std::cout << "Total registration time: " << timer.getElapsedTime() << "ms (" << iter << " iterations)" << std::endl;


    cilantro::PointCloud3f frame1_t;
    frame1_t.points.resize(3, wf.size());
    transformPointsHybrid(wf, frame1.points, frame1_t.points);

    frame1_t.points = (frame1_t.points + frame1.points)/2.0f;
    ...
    viz1.addObject<cilantro::PointCorrespondencesRenderable>("wf", frame1_t, frame1, cilantro::RenderingProperties().setLineWidth(0.1f));
...
//    viz2.addObject<cilantro::PointCloudRenderable>("frame1", frame1, cilantro::RenderingProperties().setPointColor(0,0,1).setUseLighting(false));
    viz2.addObject<cilantro::PointCloudRenderable>("frame2_t", frame1_t, cilantro::RenderingProperties().setPointColor(1,0,0).setUseLighting(false));
